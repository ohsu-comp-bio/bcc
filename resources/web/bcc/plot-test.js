...
*/
    ...
    for (var i = 0 ; i < num_tables ; i ++)
    {
        ...
        if (data_sources.hasOwnProperty(table_name))
        {
            ...
            console.log(plot_data.Date);
            plot_data = randomizeDates(plot_data);
            ...
            else
            {
                ...
                console.log("setting yaxis " + yaxes[table_name].yaxis);
                layout["yaxis" + yaxes[table_name].yaxis] = yaxis;
                if (schema.Type == "Series")
                {
                    layout["yaxis" + yaxes[table_name].yaxis].overlaying = "y5";
                }
                else
                {
                    layout["xaxis"].domain = [0.3, 1];
                    layout["yaxis" + yaxes[table_name].yaxis].overlaying = false;
                }

                if (table_name == "CA199Table")
                {
                    layout["yaxis" + yaxes[table_name].yaxis].position = 0.2;
                    layout["yaxis" + yaxes[table_name].yaxis].overlaying = "y5";
                }
...
function randomizeDates(plot_data)
{
    console.log("in randomizeDates");
    console.log("plot_data before");
    console.log(plot_data);
    var temp_dates = plot_data['Date'];
    temp_dates = temp_dates.map(function(date){return new Date(date);});
    console.log("temp_dates");
    console.log(temp_dates);

    temp_dates = temp_dates.map(function(date)
    {
        return new Date(date.setHours(Math.random()*400-200))
    });

    console.log("randomized dates");
    console.log(temp_dates);

    temp_dates = temp_dates.sort(function(d1, d2)
    {
        return d1 - d2;
    });
    ...
    plot_data['Date'] = temp_dates;
    console.log("new plot_data");
    console.log(plot_data);
